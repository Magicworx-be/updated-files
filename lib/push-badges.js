...
if (!fs.existsSync(BADGES)) {
  console.error('⚠  badges/ niet gevonden — niets te pushen (draai eerst de generator).');
  process.exit(0);
}
...
const tmp = path.join(ROOT, '.badges-push-tmp');
...
(async () => {
  ...
  if (!lokaal.length) {
    console.error('⚠  Geen badge-bestanden in badges/ — niets te pushen (draai eerst de generator).');
    process.exit(0);
  }
  ...
  try {
    ...
    // Enkel de gewijzigde/toegevoegde bestanden purgen (verwijderde hoeven niet).
    ...
    const teZuiveren = staged.filter(f => (f.endsWith('.png') || f.endsWith('.json')) && fs.existsSync(path.join(ROOT, f)));
    let ok = 0;
    ...
    console.error('⚠  Badge-push mislukt: ' + (err.stderr || err.message));
