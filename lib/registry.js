...
function loadRegistry(ROOT) {
  ...
  for (const nicheDir of fs.readdirSync(configDir, { withFileTypes: true })) {
    ...
    for (const f of fs.readdirSync(dir)) {
      ...
      const bestand = 'config/' + niche + '/' + f;
      // HARD falen, niet stil overslaan. Een config die hier wegvalt, valt uit de
      // hele keten: build-all.js bouwt haar niet (het loopt over déze lijst),
      // ruimt daarna output/<slug>/ op als "weespagina" en push-site.js haalt ze
      // van Cloudflare — alles met exitcode 0. Zo ging op 03-09-2026 een kapotte
      // config stilzwijgend een live pagina offline. De redenering "build.js meldt
      // het per slug" hield trouwens geen stand: een gedropte config bereikt
      // build.js nooit. Een onbruikbare config hoort de build te stoppen, niet
      // een pagina te laten verdwijnen. Zelfde harde-stop-stijl als loadPlannedRegions().
      let config;
      try { config = JSON.parse(fs.readFileSync(path.join(dir, f), 'utf8')); }
      catch (err) {
        throw new Error(bestand + ': ongeldige JSON — ' + err.message +
          '. Corrigeer het bestand; de build wordt bewust gestopt.');
      }
      if (!config.vak || !config.regio) {
        const mist = [!config.vak && '"vak"', !config.regio && '"regio"'].filter(Boolean).join(' en ');
        throw new Error(bestand + ': mist ' + mist + ' — elke pagina heeft beide nodig.');
      }
      const verwachteSlug = niche + '-' + regioSlugFrom(config);
      if (slug !== verwachteSlug) {
        throw new Error(bestand + ': bestandsnaam "' + slug + '" komt niet overeen met de ' +
          'regio in de config (verwacht "' + verwachteSlug + '"). Hernoem het bestand of ' +
          'corrigeer regio.kern — anders wijst de navigatie naar de verkeerde pagina.');
      }
