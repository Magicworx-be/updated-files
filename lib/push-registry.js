...
if (!TOKEN || !REPO) {
  ...
  console.error('   Vul .env aan en draai opnieuw: node lib/push-registry.js');
  console.error('   Zonder deze push zien bezoekers de nieuwe hub-kaarten niet.');
  // Exitcode 1, niet 0. De opmerking "build mag slagen zonder push" stamde uit de
  // tijd dat publiceren nog handwerk was; nu is de push de publicatie.
  process.exit(1);
}

const registryFile = process.argv[2] || path.join(ROOT, 'output', 'registry.json');
if (!fs.existsSync(registryFile)) {
  console.error('⚠  ' + registryFile + ' niet gevonden — registry.json NIET gepusht.');
  console.error('   Draai eerst: node build-all.js');
  process.exit(1);   // build-all.js schrijft dit bestand vlak ervoor; ontbreekt het, dan faalde de build
}
