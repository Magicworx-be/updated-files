...
try {
  ...
  // git schrijft zijn uitleg soms naar stdout in plaats van stderr; toon beide,
  // anders houd je een kale "Command failed" over waar je niets mee kunt.
  const uitleg = [err.stderr, err.stdout, err.message].filter(Boolean).join('\n').trim();
