...
const { normaliseerNummer } = require('./whatsapp');
...
function keurRij(r, waar, fouten) {
  ...
  // Het nummer wordt HIER genormaliseerd naar "32475123456", ongeacht in welke
  // vorm de schrijver het aanleverde ("0475 59 59 71", "+32...", "0472/092098").
  // data/whatsapp.json mag bewust elk formaat bevatten, maar het logboek voedt
  // een klikbare whatsapp://-link in het dashboard, en die wil enkel cijfers met
  // landcode. Op 4 september 2026 stond er één nummer in de ruwe vorm in — via
  // een andere weg binnengekomen dan de seed — en dat gaf een dode link.
  if (w.nummer) {
    const genormaliseerd = normaliseerNummer(w.nummer);
    if (!genormaliseerd) fouten.push(waar + ': whatsapp.nummer "' + w.nummer + '" is geen bruikbaar telefoonnummer');
    else w.nummer = genormaliseerd;
  }
